...
import com.muse.chat.repository.ConversationParticipantRepository;
import com.muse.chat.repository.ConversationRepository;
import com.muse.chat.repository.MessageRepository;
...
public class ChatService {
    ...
    public List<com.muse.chat.model.ConversationDTO> getUserConversations(String userId) {
        List<Conversation> conversations = conversationRepository.findByUserId(userId);
        return conversations.stream().map(this::toDTO).collect(java.util.stream.Collectors.toList());
    }

    public List<com.muse.chat.model.ConversationDTO> getConversationsByContext(Conversation.ContextType type,
            String contextId) {
    ...
    public com.muse.chat.model.ConversationDTO toDTO(Conversation c) {
        List<String> pIds = participantRepository.findByConversationId(c.getId()).stream()
                .map(ConversationParticipant::getUserId)
                .collect(java.util.stream.Collectors.toList());

        return com.muse.chat.model.ConversationDTO.builder()
                .id(c.getId())
                .type(c.getType())
                .name(c.getName())
