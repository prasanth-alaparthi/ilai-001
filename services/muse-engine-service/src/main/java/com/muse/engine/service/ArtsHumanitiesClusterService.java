...
public class ArtsHumanitiesClusterService {
    ...
    public Mono<VerifiableArtifact> solve(String problem) {
        return webClientBuilder.build()
                ...
                .bodyValue(Map.of("problem", problem, "cluster", "ARTS_HUMANITIES"))
                .retrieve()
                .bodyToMono(Map.class)
                .map(response -> {
                    if (Boolean.TRUE.equals(response.get("success"))) {
                        return VerifiableArtifact.builder()
                                .subject((String) response.getOrDefault("subject", "Arts & Humanities"))
                                .cluster("ARTS_HUMANITIES")
