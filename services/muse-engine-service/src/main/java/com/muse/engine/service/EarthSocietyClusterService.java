...
public class EarthSocietyClusterService {
    ...
    public Mono<VerifiableArtifact> solve(String problem) {
        return webClientBuilder.build()
                ...
                .bodyValue(Map.of("problem", problem, "cluster", "EARTH_SOCIETY"))
                .retrieve()
                .bodyToMono(Map.class)
                .map(response -> {
                    if (Boolean.TRUE.equals(response.get("success"))) {
                        return VerifiableArtifact.builder()
                                .subject((String) response.getOrDefault("subject", "Earth & Society"))
                                .cluster("EARTH_SOCIETY")
