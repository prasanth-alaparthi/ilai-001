    ...
    public ResponseEntity<Map<String, Object>> persistentSave(
        ...
        try {
            ...
            String notebookName = autoPath.getNotebook() != null ? autoPath.getNotebook()
                    : (request.getSubject() != null ? request.getSubject() + " Lab" : "Research Lab");
            ...
            // Step 4: Build metadata with variable snapshot
            Map<String, Object> metadata = new HashMap<>();
            metadata.put("type", "lab-research");
            metadata.put("subject", request.getSubject());
            metadata.put("variables", request.getVariables());
            metadata.put("equation", request.getEquation());
            metadata.put("solution", request.getSolution());
            metadata.put("createdFrom", "ResearchLab");
            ...
            note.setTags(java.util.List.of("lab-research", notebookName.toLowerCase().replace(" ", "-")));

            // Convert metadata to JsonNode
            note.setMetadata(objectMapper.valueToTree(metadata));
            ...
            response.put("variablesCaptured", request.getVariables() != null ? request.getVariables().size() : 0);
