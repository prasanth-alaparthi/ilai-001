package com.muse.parental.controller;

import com.muse.parental.client.AuthServiceClient;
import com.muse.parental.service.ParentalLockService;
...
public class ParentalLockController {
    ...
    private Mono<Long> resolveUserId(Authentication auth) {
        ...
        return authServiceClient.getUserByUsername(auth.getName())
                ...
                .onErrorResume(e -> {
                    System.err.println("ParentalLockController: Error resolving user ID for " + auth.getName() + ": "
                            + e.getMessage());
                    return Mono.empty();
                });
    }
