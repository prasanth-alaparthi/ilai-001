...
public class BountySolveOrchestrator {
    ...
    public void orchestrateSolve(Bounty bounty, Long solverId, Long creatorId, Long solutionNoteId) {
        ...
        // 1. Reputation: Award +50 points to the solver
        ...
        String solverUsername = authClient.getDisplayName(solverId); // Assuming display name is used as identifier here
        ...
        notesClient.injectSharedNote(creatorId, solutionNoteId, folderPath, solverUsername)
                .subscribe(
                        success -> log.info("Successfully linked solution note {} to creator {}'s folder",
                                solutionNoteId, creatorId),
                        error -> log.error("Failed to link solution note to creator folder: {}", error.getMessage()));
